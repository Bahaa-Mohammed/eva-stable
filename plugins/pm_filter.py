...
@Client.on_message(filters.group & filters.incoming) #filters.text
...
async def filter(client, message):  
    ...
    if 2 < len(message.text) < 100:    
        btn = []
        search = message.text
        files, total_results = await get_search_results(search.lower(), filter=True)  
        if files:
            for file in files:
...
async def advantage_spoll_choker(bot, query):
    ...
    if k==False:
        files, total_results = await get_search_results(movie, filter=True)
        if files:
            k = (movie, files, total_results)
    ...
    if (clicked == typed) or (clicked in AUTH_USERS) or (clicked in ADMINS):   
        ...
        if query.data.startswith("next"):
            ...
            if query.message.chat.id not in ADMINS:
                await query.answer("❌ No Tresspassing. No Excuses. Authorized Personnel Only. There is 🐕 Dog Inside and Refuses to take His Medication. Stay Away 🚷", show_alert=True)
                return
            else:
                buttons = [
                    [
